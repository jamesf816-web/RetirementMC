...
class RetirementSimulator:
    ...
    def run_simulation(self):
        ...
        if self.accounts is None:
            raise ValueError("You must pass the 'self.accounts' dictionary with initial balances and account info.")
        ...
        self.portfolio_end = np.zeros(n_sims)
        self.conversion_paths = np.zeros((n_sims, n_full))
        self.portfolio_end = np.zeros(n_sims)
        self.portfolio_end = np.zeros(n_sims)
        self.travel_paths = np.zeros((n_sims, n_full))
        self.portfolio_end = np.zeros(n_sims)
        self.gifting_paths = np.zeros((n_sims, n_full))
        self.portfolio_end = np.zeros(n_sims)
        self.base_spending_paths = np.zeros((n_sims, n_full))
        self.portfolio_end = np.zeros(n_sims)
        self.lumpy_spending_paths = np.zeros((n_sims, n_full))
        self.rmd_withdrawal = {name: 0.0 for name in account_names}
        self.plan_paths = np.zeros((n_sims, n_full))
        self.taxes_paths = np.zeros((n_sims, n_full))
        self.trust_income_paths = np.zeros((n_sims, n_full))
        self.ssbenefit_paths = np.zeros((n_sims, n_full))
        self.portfolio_withdrawal_paths = np.zeros((n_sims, n_full))
        self.rmd_paths = np.zeros((n_sims, n_full))
        self.def457b_income_paths = np.zeros((n_sims, n_full))
        self.pension_paths = np.zeros((n_sims, n_full))
        self.medicare_paths = np.zeros((n_sims, n_full))
        ...
        for sim_idx in range(n_sims):
            ...
            accounts_bal = {name: acct["balance"] for name, acct in self.accounts.items()}
            ...
            for name in account_names:
                self.account_paths[name][sim_idx,0] = accounts_bal[name]
            ...
            self.portfolio_paths[sim_idx, 0] = starting_balance # just use prior year as estimate (not used unless plots extended back)
            self.portfolio_paths[sim_idx, 1] = starting_balance # 
            
            #self.debug_log.append(f"Starting Balance = ${starting_balance:,.0f} sim_idx = sim_idx \n")

            # 3. Initialize MAGI for prior 2 years
            self.magi_paths[sim_idx,0] = self.magi_2  
            self.magi_paths[sim_idx,1] = self.magi_1 
 
            # --- Initialize quarter index at FY2026Q1 = quarter_index = 8 ---
            quarter_index = int((self.current_year - (self.years[0] + 2)) * 4)   # dynamic
            #quarter_index = 8
            ...
            for y, year in enumerate(range(self.current_year, self.current_year + n_years_sim)):
                ...
                #----------------------
                # Record what the values of required RMDs are for each  account - funds will come out of self.accounts quarterly
                #----------------------
                rmds = 0.0
                rmd_withdrawal = {}
                ...
                for acct_name, acct in self.accounts.items():
                    ...
                    if acct["tax"] == "traditional":
                        ...
                        mand = acct_balance / rmd_factor if rmd_factor > 0 else 0
                        rmds += mand
                self.rmd_paths[sim_idx,year_idx] = rmds
                ...
                estimated_tax = 0.0
                portfolio_start = self.portfolio_paths[sim_idx,year_idx-1] #start year with ending balance of prior year
                ...
                self.pension_paths[sim_idx,year_idx] = pension_income
                ...
                #----------------------
                # Record what the values of required 457b are for each  account - funds will come out of self.accounts quarterly
                #----------------------
                def457b_withdrawal = {}
                def457b = 0.0 #running total over all 457b self.accounts
                ...
                self.def457b_income_paths[sim_idx,year_idx] = def457b
                ...
                self.base_spending_paths[sim_idx,year_idx] = base_spend
                self.lumpy_spending_paths[sim_idx,year_idx] = lumpy_expenses
                ...
                tax_result = calculate_taxes(
                    ...
                    magi_two_years_ago=self.magi_paths[sim_idx,year_idx-2],# note that 2024, 2025 were added at front of array
                ...
                self.travel_paths[sim_idx,year_idx] = actual_travel
                ...
                # person2
                person = "person2"
                trad_accts = {k: v for k, v in accounts_bal.items() if self.accounts[k]["owner"] == person and self.accounts[k]["tax"] == "traditional"}
                trad_total = sum(trad_accts.values())
                roth_accts = {k: v for k, v in accounts_bal.items() if self.accounts[k]["owner"] == person and self.accounts[k]["tax"] == "roth"}
                ...
                self.conversion_paths[sim_idx,year_idx] = conv_jef + conv_sef
                ...
                for q in range(4):
                    ...
                    # Then take the rest in order by self.accounts        
                ...
                # At this point ROTH conversions and gifting remain to be taken from self.accounts in Q4
                #-------------------------
                # Execute ROTH conversions in Q4 (move funds from Trad to Roth)
                #-------------------------
                person = "person2"
                # Re-compute at time of conversion (Q4)
                trad_accts = {k: accounts_bal[k] for k, v in self.accounts.items()
                if v["owner"] == person and v["tax"] == "traditional" and accounts_bal[k] > 0}
                roth_accts = {k: accounts_bal[k] for k, v in self.accounts.items() 
                ...
                self.gifting_paths[sim_idx,year_idx] = actual_gifting
                
                # Withdraw gifting from self.accounts (track taxable portions)
                ...
                self.magi_paths[sim_idx,year_idx] = magi
                ...
                if total_tax > total_estimated_tax: # then we need to draw funds to cover
                    ...
                    # Withdraw shortfall from self.accounts
                    remaining = shortfall
                ...
                self.portfolio_withdrawal_paths[sim_idx,year_idx] = portfolio_draws  # Exclusive of RMDs amd Trust Income
        ...
        result = {
            ...
            "travel_paths": self.travel_paths,
            "gifting_paths": self.gifting_paths,
            ...
            "def457b_income_paths": self.def457b_income_paths,
